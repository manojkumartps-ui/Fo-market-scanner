...
def check_stock(df, debug=False):
    ...
    conditions = [
        ("Daily Price > Open", latest["Close"] > latest["Open"]),
        ("Daily Close > EMA_HA_Close", latest["Close"] > latest["EMA_HA_Close"]),
        ("Daily Trend Bullish", latest["EMA_HA_Open"] < latest["EMA_HA_Close"]),
        ("Daily Lookback Bearish", d_3["EMA_HA_Open"] > d_3["EMA_HA_Close"]),
        ("Weekly Price > Open", w_latest["Close"] > w_latest["Open"]),
        ("Weekly Close > EMA_HA_Close", w_latest["Close"] > w_latest["EMA_HA_Close"]),
        ("Weekly Trend Bullish", w_latest["EMA_HA_Open"] < w_latest["EMA_HA_Close"]),
        ("Weekly Lookback Bearish", w_3["EMA_HA_Open"] > w_3["EMA_HA_Close"])
    ]

    if debug:
        print("--- LOGIC TRACE (First Stock) ---")
    ...
    try:
        ...
        for stock in tickers:
            ...
            try:
                ...
                # Use debug for the very first stock to see why it's failing
                if check_stock(df, debug=first_check):
